...
/**
 * La carte de la commune, chargée au clic seulement.
 * ---------------------------------------------------------------------------
 * Une carte intégrée interroge les serveurs d'OpenStreetMap dès l'ouverture
 * de la page, et leur transmet l'adresse IP du visiteur. Pour une
 * collectivité, c'est une transmission de données à un tiers sans base
 * légale : le consentement se donne, il ne se présume pas.
 *
 * Sans JavaScript, le bouton ne fait rien et la liste des lieux reste
 * lisible : c'est elle qui porte l'information, la carte ne fait que la
 * situer.
 */
(function () {
	'use strict';

	var facades = document.querySelectorAll('.carte-facade[data-carte]');
	if (!facades.length) { return; }

	Array.prototype.forEach.call(facades, function (facade) {
		var bouton = facade.querySelector('.carte-afficher');
		if (!bouton) { return; }

		bouton.addEventListener('click', function () {
			var cadre = document.createElement('iframe');
			cadre.setAttribute('src', facade.getAttribute('data-carte'));
			cadre.setAttribute('title', bouton.textContent.trim());
			cadre.setAttribute('loading', 'lazy');
			cadre.setAttribute('referrerpolicy', 'no-referrer');

			var fond = facade.querySelector('.carte-fond');
			if (fond) { fond.remove(); }
			bouton.remove();
			facade.insertBefore(cadre, facade.firstChild);
		});
