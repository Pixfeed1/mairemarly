...
/**
 * Onglet « Accès rapides » et panneau latéral.
 * ---------------------------------------------------------------------------
 * L'onglet est collé au bord droit de l'écran, sur toutes les pages. Le
 * panneau se déploie vers la gauche par-dessus la page courante : chercher un
 * raccourci ne doit pas faire perdre l'endroit où l'on se trouvait.
 *
 * Sans JavaScript, un bouton qui n'ouvre rien est un mensonge : le CSS le
 * masque tant que cette fonction ne s'est pas exécutée (.js-actif). Rien
 * n'est perdu, le pied de page reprend l'intégralité de ces destinations.
 */
(function () {
	'use strict';

	var onglet  = document.querySelector('.onglet-acces');
	var panneau = document.getElementById('panneau-acces');
	var entete  = document.querySelector('.entete');
	if (!onglet || !panneau) { return; }

	/* Le panneau se cale sous l'en-tête. On mesure ici aussi, sans dépendre
	   du bloc précédent : ce module doit fonctionner même si le menu mobile
	...
	/* Le voile n'est pas dans le HTML : sans JavaScript il n'aurait rien à
	   couvrir. On le fabrique ici, au moment où il devient utile. */
	var voile = document.createElement('div');
	voile.className = 'voile-acces';
	voile.setAttribute('data-ouvert', 'non');
	...
	function basculer(vers) {
		...
		onglet.setAttribute('aria-expanded', ouvert ? 'true' : 'false');
		panneau.setAttribute('data-ouvert', ouvert ? 'oui' : 'non');
		panneau.setAttribute('aria-hidden', ouvert ? 'false' : 'true');
		voile.setAttribute('data-ouvert', ouvert ? 'oui' : 'non');
		if ('inert' in HTMLElement.prototype) { panneau.inert = !ouvert; }
		document.documentElement.style.overflow = ouvert ? 'hidden' : '';

		if (ouvert) {
			var premier = panneau.querySelector('button, a[href]');
			if (premier) { premier.focus(); }
		} else {
			onglet.focus();
		}
	...
	onglet.addEventListener('click', function () { basculer(); });
	voile.addEventListener('click', function () { basculer(false); });

	var fermeture = panneau.querySelector('[data-ferme-acces]');
	if (fermeture) {
		fermeture.addEventListener('click', function () { basculer(false); });
	}
